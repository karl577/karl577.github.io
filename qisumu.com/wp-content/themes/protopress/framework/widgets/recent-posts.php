...
class protopress_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'protopress_rp', // Base ID
			__('ProtoPress Recent Posts with Thumbnails', 'protopress'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'protopress' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
		...
		$title = __('Latest Posts','protopress');
		$no_of_posts = 5;
	endif;
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'protopress' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'protopress' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','protopress' ); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'protopress' ); ?></label>
...
add_action( 'widgets_init', 'register_protopress_widget' );  
function register_protopress_widget() {  
    register_widget( 'protopress_Recent_Posts' );  
}  
