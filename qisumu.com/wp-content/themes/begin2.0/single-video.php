		...
		<main id="main" class="site-main" role="main">
			...
			<?php while ( have_posts() ) : the_post(); ?>
					...
					<div class="entry-content">
						<div class="single-content">
							...
							<div class="videos-content">
								...
								<div class="video-inf">
									<ul class="date">日期：<?php the_time('Y年m月d日') ?></ul>
									<ul class="category">分类：<?php echo get_the_term_list($post->ID,  'videos', '', ', ', ''); ?></ul>
									<?php if ( post_password_required() ) { ?>
										<ul class="comment">评论：<a href="#comments">密码保护</a></ul>
									<?php } else { ?>
										<ul class="comment">评论：<?php comments_popup_link( '发表评论', '1 条', '% 条' ); ?></ul>
									<?php } ?>
									<ul class="comment"><?php if( function_exists( 'the_views' ) ) { the_views( true, '观看：  ',' 次' ); } ?></ul>
								</div>
								<div class="clear"></div>
							</div>
						...
						<footer class="single-footer">
							...
							<div class="single-cat-tag">
								<div class="single-cat">日期：<?php the_time( 'Y年m月d日' ) ?> 分类：<?php echo get_the_term_list( $post->ID,  'videos', '' ); ?></div>
								<div class="single-tag"><?php echo get_the_term_list($post->ID,  'videotag', '标签：', ', ', ''); ?></div>
							</div>
						</footer><!-- .entry-footer -->
				...
				<?php if (zm_get_option('related_img')) { ?>
					<div class="wow fadeInUp" data-wow-delay="0.3s"><?php get_template_part( 'template/related-video' ); ?></div>
				<?php } ?>
