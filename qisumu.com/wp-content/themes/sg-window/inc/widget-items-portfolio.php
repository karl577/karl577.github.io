...
class sgwindow_items_portfolio extends WP_Widget {
	/**
	 * Widget constructor
	 *
	 * @since SG Window 1.0.0
	...
	*/
	public function __construct() {
		...
		/* Widget settings. */
		$widget_ops = array(
		'classname' => 'sgwindow_items_portfolio',
		'description' => __('Display Items from Portfolio.', 'sg-window' ));

		/* Widget control settings. */
		$control_ops = array(
		'width' => 250,
		'height' => 250,
		'id_base' => 'sgwindow_items_portfolio_widget');


		/* Create the widget. */		
		parent::__construct( 'sgwindow_items_portfolio_widget', __( 'SG Items from Portfolio (SG Window)', 'sg-window' ), $widget_ops, $control_ops );
			...
			<div class="main-wrapper-image <?php echo $pos_class;?>" style="padding:<?php echo esc_attr($instance['padding_top']);?>px <?php echo esc_attr($instance['padding_right']);?>% <?php echo esc_attr($instance['padding_bottom'])?>px <?php echo esc_attr($instance['padding_left'])?>%">
				...
				<div class="description">
					<article>
						...
						<a href="<?php echo esc_url( get_term_link( $instance['jetpack-portfolio-type'], $tax ) ); ?>" class="link" rel="bookmark"><?php _e('Open Archive', 'sg-window'); ?></a>
					</article> <!-- article -->
				</div> <!-- .description -->
	...
	*/
	function form( $instance ) {
		...
		if ( $terms && ! is_wp_error( $terms ) ) : 
			...
			<select id="<?php echo esc_attr( $this->get_field_id('jetpack-portfolio-type') ); ?>" name="<?php echo esc_attr( $this->get_field_name('jetpack-portfolio-type') ); ?>" style="width:100%;">
	...
	function defaults( $instance ){
		...
		$defaults = array('title' =>  __('Recent Projects', 'sg-window'),
 ...
 *
 */
function sgwindow_register_items_portfolio_widgets() {
	register_widget( 'sgwindow_items_portfolio' );
}
add_action( 'widgets_init', 'sgwindow_register_items_portfolio_widgets' );
