 ...
 */
add_action( 'customize_register', 'sgwindow_customize_register_fonts' );
function sgwindow_customize_register_fonts( $wp_customize ) {

	$defaults = sgwindow_get_defaults();
	
	class sgwindow_Customize_Reset_Control extends WP_Customize_Control {
		public $type = 'button';
		...
		public function render_content() {
			...
			<form>
				...
				<input type="button" onclick="sgwindow_refresh()" value="<?php echo esc_attr( $this->value() ); ?>" <?php $this->link(); ?> />
			</form>
			<?php
		}
	...
	$wp_customize->add_panel( 'fonts', array(
		'priority'       => 105,
		'title'          => __( 'Customize Fonts', 'sg-window' ),
		'description'    => __( 'Selecting a google fonts.', 'sg-window' ),
	) );
...
//New section in customizer: Fonts
	$wp_customize->add_section( 'fonts', array(
		'title'          => __( 'Google Fonts', 'sg-window' ),
...
//Font for site
	$wp_customize->add_setting( 'body_font', array(
		...
		'sanitize_callback' => 'sgwindow_sanitize_font_choice'
	) );
	$wp_customize->add_control( 'body_font', array(
		'label'   => __( 'Body Font', 'sg-window' ),
		'section' => 'fonts',
		'settings'   => 'body_font',
		'type'       => 'select',
		'priority'   => 2,
		'choices'	 => sgwindow_get_font_choices(),
	) );	
...
//Header and description
	...
	$wp_customize->add_control( 'header_font', array(
		'label'   => __( 'Header Font', 'sg-window' ),
		'section' => 'fonts',
...
//Font for headings
	...
	$wp_customize->add_control( 'heading_font', array(
		'label'   => __( 'Heading Font', 'sg-window' ),
		'section' => 'fonts',
...
//Font for headings
	...
	$wp_customize->add_control( 'heading_font_size', array(
		'label'      => __( 'Heading font size(px)', 'sg-window' ),
		'section'    => 'fonts_size',
...
//Font weight for headings
	$wp_customize->add_setting( 'heading_weight', array(
		...
		'capability'     => 'edit_theme_options',
		'sanitize_callback' => 'sgwindow_sanitize_weight_choice'
	) );
	$wp_customize->add_control( 'heading_weight', array(
		'label'   => __( 'Heading Font Weight', 'sg-window' ),
		'section' => 'fonts',
		'settings'   => 'heading_weight',
		'type'       => 'select',
		'priority'   => 2,
		'choices'	 => sgwindow_get_weight_choices(),
	) );
	
//New section in customizer: Reset
	$wp_customize->add_section( 'reset', array(
		'title'          => __( 'Reset', 'sg-window' ),
		'description'          => __( 'Reset font settings to default', 'sg-window' ),
		'priority'       => $section_priority++,
		'panel'  => 'fonts',
	) );
	...
	$wp_customize->add_setting( 'reset_fonts', array(
		'type'			 => 'empty',
		'default'        => __( 'Reset', 'sg-window' ),
		'sanitize_callback' => 'sgwindow_sanitize_url'
	) );
	
	$wp_customize->add_control( new sgwindow_Customize_Reset_Control( $wp_customize, 'reset_fonts', array(
		'label'   => 'reset_fonts',
		'section' => 'reset',
 ...
 */
function sgwindow_get_weight_choices() {

	$out = array( 'lighter' => __( 'Lighter', 'sg-window' ), 
				  'normal' => __( 'Normal', 'sg-window' ), 
				  'bold' => __( 'Bold', 'sg-window' ));
	return $out;
}
 ...
 * @since SG Window 1.0.1
 */
function sgwindow_sanitize_weight_choice( $value ) {

	if ( array_key_exists( $value, sgwindow_get_weight_choices() ) ) {
		return $value;
	} else {
		return 'lighter';
	}
 ...
 * @since SG Window 1.0.0
 */
function sgwindow_sanitize_font_choice( $value ) {
	if ( '0' == $value ) {
		return '0';
	} else if ( array_key_exists( $value, sgwindow_get_font_choices() ) ) {
		return $value;
	} else {
		return 'Open Sans';
	}
 ...
 * @since SG Window 1.0.0
 */
function sgwindow_get_font_choices() {
	$fonts = sgwindow_get_google_fonts();
	$out = array();
...
if ( ! function_exists( 'sgwindow_get_google_fonts' ) ) :
/**
 ...
 * @since SG Window 1.0.0
 */
function sgwindow_get_google_fonts() {
	return array(
		'0' => array(
			'label'    => __( 'None', 'sg-window' ), 
		),		
		'ABeeZee' => array(
			'label'    => 'ABeeZee',
			'variants' => array(
...
?>
	...
	h5,
	h6 {
		font-weight: <?php echo esc_attr( sgwindow_get_theme_mod( 'heading_weight' ) ); ?>;
	...
	@media screen and (min-width: 680px) {
		h1 {
			font-size:<?php echo esc_attr( sgwindow_get_theme_mod( 'heading_font_size' ) ); ?>px;
		}
	}
...
add_action( 'wp_head', 'sgwindow_hook_font_css' );
 ...
 * @since SG Window 1.0.1
*/
add_action( 'customize_controls_print_scripts', 'sgwindow_print_customizer_js_colors' );
 ...
 * @since SG Window 1.0.1
*/
function sgwindow_print_reset( $fields, $bind ) {

?>
<script type="text/javascript">
	
	function sgwindow_set_val(name, newVal) {
	...
	function sgwindow_refresh() { 
	
	<?php 
		foreach( $fields as $id => $value ) {
			?>
			sgwindow_set_val( '<?php echo esc_js( $id ); ?>', '<?php echo esc_js( $value ); ?>' );
			<?php
		}	
	?>
	}
