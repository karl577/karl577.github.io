 ...
 * Add breadcrumb.
 *
 * @action jomsom_after_header
 *
 * @since Jomsom 0.1
 */
if( !function_exists( 'jomsom_add_breadcrumb' ) ) :

	function jomsom_add_breadcrumb() {
		...
		if( isset ( $options['breadcumb_option'] ) && $options['breadcumb_option'] ) {
			...
			echo jomsom_custom_breadcrumbs( $showOnHome, $delimiter );
		}
		else
			return false;
	}
...
add_action( 'jomsom_after_header', 'jomsom_add_breadcrumb', 50 );
 ...
 * @since Jomsom 0.1
 */
if( !function_exists( 'jomsom_custom_breadcrumbs' ) ) :

	function jomsom_custom_breadcrumbs( $showOnHome, $delimiter ) {

		/* === OPTIONS === */
		$text['home']     = __( 'Home', 'jomsom' ); // text for the 'Home' link
		$text['category'] = __( '%1$s Archive for %2$s', 'jomsom' ); // text for a category page
		$text['search']   = __( '%1$s Search results for: %2$s', 'jomsom' ); // text for a search results page
		$text['tag']      = __( '%1$s Posts tagged %2$s', 'jomsom' ); // text for a tag page
		$text['author']   = __( '%1$s View all posts by %2$s', 'jomsom' ); // text for an author page
		$text['404']      = __( 'Error 404', 'jomsom' ); // text for the 404 page
		...
		if( is_front_page() ) {
			...
			if( get_query_var( 'paged' ) ) {
				...
				echo '<span class="breadcrumb-paged">' . $delimiter . sprintf( __( 'Page %s', 'jomsom' ), max( $paged, $page ) ) . '</span>';
				if( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
					echo ')';
				}
			}
