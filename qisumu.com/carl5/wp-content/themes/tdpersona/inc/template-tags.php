...
function tdpersona_content_nav( $nav_id ) {
	...
	<?php if ( is_single() ) : // navigation links for single posts ?>
		...
		<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . _x( '<i class="fa fa-angle-double-left"></i>', '上一篇: %link', 'tdpersona' ) . '</span> %title' ); ?>
		<?php next_post_link( '<div class="nav-next">%link</div>', '%title <span class="meta-nav">' . _x( '<i class="fa fa-angle-double-right"></i>', '下一篇: %link', 'tdpersona' ) . '</span>' ); ?>

	<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>

		<?php if ( get_next_posts_link() ) : ?>
		<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav"><i class="fa fa-angle-double-left"></i></span> 较旧的贴', 'tdpersona' ) ); ?></div>
		<?php endif; ?>

		<?php if ( get_previous_posts_link() ) : ?>
		<div class="nav-next"><?php previous_posts_link( __( '较新的贴 <span class="meta-nav"><i class="fa fa-angle-double-right"></i></span>', 'tdpersona' ) ); ?></div>
		<?php endif; ?>
...
function tdpersona_posted_on() {
	...
	if ( ! post_password_required() && ( comments_open() || '0' != get_comments_number() ) ) {
		ob_start();
		$comments_link = comments_popup_link( __( '发表评论', 'tdpersona' ), __( '1 条评论', 'tdpersona' ), __( '%s 条评论', 'tdpersona' ) );
	...
	printf( __( '
		...
		esc_attr( sprintf( __( '查看 %s 所有的贴子', 'tdpersona' ), get_the_author() ) ),
		get_the_author(),
		$categories_list,
		$comments
	);
