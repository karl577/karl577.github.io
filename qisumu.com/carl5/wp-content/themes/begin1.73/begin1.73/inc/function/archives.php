...
function cx_archives_list() {
	if( !$output = get_option('cx_archives_list') ){
		...
		while ( $the_query->have_posts() ) : $the_query->the_post();
			...
			$output .= '<li><i class="icon-li"></i>'. get_the_time('d日: ') .'<a href="'. get_permalink() .'">'. get_the_title() .'</a> <span class="ac"><i class="icon-scroll-c"></i> '. get_comments_number('0', '1', '%') .'</span></li>';
		endwhile;
		wp_reset_postdata();
		$output .= '</ul></li></ul></div>';
