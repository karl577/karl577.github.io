...
class extend_forum_upload {
	...
	function extend_forum_upload($getaid = 0) {
		global $_G;
		...
		if($_POST['hash'] != $swfhash) {
			return $this->uploadmsg(10);
		}


		$upload = new extend_discuz_upload();
		$_POST['tmp_name'] = $_POST['key'] ;
		$upload->init($_POST, 'forum');
		$this->attach = &$upload->attach;
		//var_dump($this->attach);
		if($upload->error()) {
		...
		if($upload->attach['isimage']) {
			...
			if($_G['setting']['thumbsource'] || !$_G['setting']['thumbstatus']) {
				list($width) =  $upload->attach['imageinfo'];
			}
		}
		if($_GET['type'] != 'image' && $upload->attach['isimage']) {
		...
		//var_dump(dhtmlspecialchars(censor($upload->attach['name'])));exit;
		$insert = array(
			'aid' => $aid,
