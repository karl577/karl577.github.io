 ...
 * Document   : function_wechatlogin.php
 * Created on : 2016-8-4 21:27:39
 * Author     : 老虎(Tiger)
...
if (!function_exists('HaoTeamIconv')) {
     ...
     * @return type
     */
    function HaoTeamIconv($original) {
...
function getHaoteamButtonLink($mod) {
    global $_G;
    ...
    $data = array(
        ...
        'partner' => $_G['cache']['plugin']['tiger_wechatlogin']['partner'],
        'sign' => md5(md5($_G['cache']['plugin']['tiger_wechatlogin']['partner'] . $_G['cache']['plugin']['tiger_wechatlogin']['key'])),
        'type' => $type,
        'wap' => checkmobile(),
        'clientapp' => 2,
        'return_url' => $_G['siteurl'] . 'plugin.php?id=tiger_wechatlogin&mod=' . $mod . '&referer=' .  urlencode($_GET['referer']), //同步回调地址
    );

    ksort($data);
    return $_G['cache']['plugin']['tiger_wechatlogin']['service'] . http_build_query($data);
}
 ...
 * @param type $member
 */
function _silenceregister($member) {
    global $_G;
    ...
    $defaultusername = _Hclear($member['nickname']);
    //是否快速注册
    if (empty($_G['cache']['plugin']['tiger_wechatlogin']['wechat_allowfastregister'])) {
        _Hredirectregister($defaultusername);
    }
    ...
    $uid = Hregister($defaultusername, 1, 0);
    if (!$uid) {
        _Hredirectregister($defaultusername);
    }else{
        HbindOpenId($uid, $member['unionid'], 1);
    }
}
 ...
 * @return boolean
 */
function Hcheck_openid_auth($openid, $unionid) {
    global $_G;
    $uid = DB::fetch_first("SELECT uid FROM " . DB::table('common_member_wechat') . " WHERE openid = '" . intval($openid) . "' OR openid = '" . addslashes($unionid) . "'");
    if (empty($uid['uid'])) {
        return false;
    } else {
        $_G['member'] = getuserbyuid($uid['uid'], 1);
        if ($_G['member']) {
            DB::update('common_member_wechat', array('openid' => addslashes($unionid)), "openid = '" . intval($openid) . "' OR openid = '" . addslashes($unionid) . "'");
            require_once libfile('function/member');
            setloginstatus($_G['member'], 1296000);
        }
        return true;
    }
}
 ...
 */
function HbindOpenId($uid, $unionid, $isregister = 0) {
    C::t('#wechat#common_member_wechat')->insert(array('uid' => $uid, 'status' => 2, 'openid' => addslashes($unionid), 'isregister' => intval($isregister)), false, true);
}
 ...
 * @return type
 */
function Hregister($username, $return = 0, $groupid = '') {
    global $_G;
    if (!$username) {
        return;
    }
    ...
    $groupid = empty($groupid) ? ($_G['cache']['plugin']['tiger_wechatlogin']['wechat_newusergroupid'] ? $_G['cache']['plugin']['tiger_wechatlogin']['wechat_newusergroupid'] : $_G['setting']['newusergroupid']) : intval($groupid);
    ...
    //是否忽略注册规则
    if (!$_G['cache']['plugin']['tiger_wechatlogin']['wechat_disableregrule']) {
        loadcache('ipctrl');
    ...
    C::t('common_member')->insert(intval($uid), addslashes($username), $password, $email, $_G['clientip'], intval($groupid), $init_arr);
 ...
 * @param type $username
 */
function _Hredirectregister($username) {
    global $_G;
    ...
    $auth = urlencode(base64_encode(authcode($_GET['unionid'] . "\t" . $_GET['openidsign'] . "\t" . $_GET['referer'], 'ENCODE')));
    $referer = urlencode($_G['siteurl'] . 'plugin.php?id=tiger_wechatlogin&mod=login&ac=regcallback&auth=' . $auth);
