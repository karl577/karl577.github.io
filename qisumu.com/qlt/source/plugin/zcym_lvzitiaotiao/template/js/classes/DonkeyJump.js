...
(function() {
    ...
    DonkeyJump.prototype.__createUI = function() {
        ...
        ui.onretry = function() {
            ...
            this.toBody();
            ui.load_data('plugin.php?id=zcym_lvzitiaotiao:ajax',function(res){
            	var _data = res.split("|");
        		if(_data[0]==0){
        			showDialog(_data[1]);
        		}else{
        			document.getElementById("_trfreshtoken").innerHTML = _data[1];
    ...
    DonkeyJump.prototype.gameover = function() {
        ...
        this.stop();
        ui.updateResult(document.getElementById('zcym_game_name').innerHTML, this.score);
        var getor = new Ajax("HTML");
		getor.get('plugin.php?id=zcym_lvzitiaotiao:ajax&rt='+Math.random()+'&gamehash='+game_hash+'&__jrefreshgametine__='+this.score*1024/2+'&rtjs_dgsdgsdj='+document.getElementById("_trfreshtoken").innerHTML,function(res){
			var _data = res.split("|");
    		if(_data[0]==0){
    			showDialog(_data[1]);
    		}else{
    			var _game_score = parseInt(document.getElementById("zcym_game_score").innerHTML,10);
    			var _game_pm = document.getElementById("zcym_game_pm").innerHTML;
    			_game_pm = isNaN(_game_pm) ? 0 : parseInt(_game_pm,10);
    			if(_game_score<_data[1]){
    				document.getElementById("zcym_game_score").innerHTML = _data[1];
    			}
    			if(_game_pm<_data[2]){
    				document.getElementById("zcym_game_pm").innerHTML = _data[2];
    			}
    		}
			ui.toOver();
    	});
